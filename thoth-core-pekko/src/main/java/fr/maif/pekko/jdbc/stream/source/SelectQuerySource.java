			...
			public void postStop() {
				try {
					if (Boolean.TRUE.equals(closeConnection)) {
						LOGGER.debug("[Pekko stream JDBC select source] - Closing connexion");
					}
					cleanResources(statement, sqlContext.connection, closeConnection);
				} catch (SQLException e) {
					LOGGER.error("[Pekko stream JDBC select source] - Error closing connexion", e);
				}
			}

			{
				setHandler(out, new AbstractOutHandler() {
					...
					public void onPull() throws Exception {
						try {
							if (!executed) {
								log.debug("[Pekko stream JDBC select source] - Preparing statement for query {} ", query);
								PreparedStatement preparedStatement = query.buildPreparedStatement(sqlContext.connection);
								statement = preparedStatement;
								log.debug("[Pekko stream JDBC select source] - Executing query {} ", query);
							...
							LOGGER.error("[Pekko stream JDBC select source] - Error executing request {} with params {}", query.sql(), query.params());
